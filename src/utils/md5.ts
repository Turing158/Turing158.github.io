// 纯 JS 实现的 MD5（基于 RFC 1321），支持 UTF-8 字符串
// MD5 是单向哈希算法，理论上不可逆，因此本工具仅提供「编码」能力。
// 「解密」入口将通过查询公共彩虹表 API 实现。
...
function md51(bytes: Uint8Array): number[] {
  ...
  const remain = bytes.subarray(i - 64)
  let j = 0
  for (; j < remain.length; j++) {
    tail[j >> 2] |= remain[j] << ((j % 4) * 8)
  }
  tail[j >> 2] |= 0x80 << ((j % 4) * 8)
  if (j > 55) {
    md5cycle(state, tail)
  ...
  const bitLen = n * 8
  tail[14] = bitLen >>> 0
  tail[15] = Math.floor(n / 0x20000000) // n * 8 高 32 位 ≈ n / (2^29)
  md5cycle(state, tail)
  return state
}
...
function rhex(num: number): string {
  let s = ''
  for (let j = 0; j < 4; j++) {
    s += ((num >> (j * 8 + 4)) & 0x0f).toString(16)
    s += ((num >> (j * 8)) & 0x0f).toString(16)
  }
  return s
}
